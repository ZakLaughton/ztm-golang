...
func (s *Swatch) SetColor(c color.Color) {
	s.Color = c
	// Refresh the widget whenever the color changes
	s.Refresh()
}
...
func NewSwatch(state *apptype.State, color color.Color, swatchIndex int, clickHandler func(s *Swatch)) *Swatch {
	...
	// part of the widget.BaseWidget interface
	// supplies all the state information to the find toolkit so we don't have to do it manually
